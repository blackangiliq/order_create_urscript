...
### إضافة order.created للقائمة المنسدلة
الإضافة تقوم تلقائياً بإضافة خيار "إنشاء طلب جديد (Order Created)" إلى القائمة المنسدلة في modal الويب هوك، حتى تتمكن من:
- اختيار order.created من القائمة يدوياً
- إنشاء webhook عادي من خلال الواجهة
- الاستفادة من جميع الخيارات المتاحة (Rules، Headers، إلخ)
...
### إضافة أحداث أخرى
يمكنك إضافة أحداث أخرى للقائمة المنسدلة بتعديل دالة `addOrderCreatedToDropdown()` في `content.js`.
